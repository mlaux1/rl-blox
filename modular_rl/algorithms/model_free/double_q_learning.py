    ...
    while not terminated and not truncated:
        ...
        if val < 0.5:
            q_table1 = _dql_update(
                key, q_table1, q_table2, observation, action, reward,
                next_observation, gamma, alpha)
        else:
            q_table2 = _dql_update(
                key, q_table2, q_table1, observation, action, reward,
                next_observation, gamma, alpha)
...
@jit
def _dql_update(key, q_table1, q_table2, observation, action, reward, next_observation, gamma, alpha):
